...
export default function AssignmentFlow() {
  ...
  const { workers, complaints, updateComplaint } = useMockStore();
  ...
  const handleAssign = () => {
    ...
    // Simulate tactical relay
    setTimeout(() => {
      updateComplaint(selectedTaskId, {
        workerId: id,
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="max-w-6xl mx-auto w-full p-6 md:p-12 flex flex-col gap-10">
         ...
         <div className="flex flex-col md:flex-row gap-6 justify-between items-center">
            <div className="relative w-full md:max-w-md group">
               ...
               <input 
                  ...
                  className="w-full h-16 bg-white border border-slate-200 rounded-[2rem] pl-16 pr-8 text-sm font-medium outline-none focus:border-primary shadow-xl shadow-slate-100 transition-all"
               />
            </div>
            <div className="flex gap-4 w-full md:w-auto">
               <button className="flex-1 md:flex-none h-16 px-8 bg-white border border-slate-200 rounded-[2rem] flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-all">
                  <Filter className="w-4 h-4" /> Category Filter
               </button>
               <button className="flex-1 md:flex-none h-16 px-8 bg-slate-900 text-white rounded-[2rem] flex items-center gap-3 text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableTasks.map(task => (
               <div 
                  ...
                  className={`bg-white p-8 rounded-[3rem] border-2 transition-all cursor-pointer relative overflow-hidden flex flex-col gap-6 shadow-xl shadow-slate-200/40 ${selectedTaskId === task.id ? 'border-primary shadow-orange-200' : 'border-slate-100 opacity-60 hover:opacity-100 hover:border-slate-200'}`}
               >
                  ...
                  <div className="flex flex-col gap-2">
                     ...
                     <h3 className="text-xl font-black text-slate-900 tracking-tight leading-tight">{task.title}</h3>
                  </div>

                  <div className="space-y-3">
                     <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500">
                        <MapPin className="w-4 h-4 text-primary" /> {task.address?.substring(0, 30)}
                     </div>
                     <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500">
                        <Clock className="w-4 h-4 text-primary" /> {new Date(task.createdAt).toLocaleTimeString()}
                     </div>
                  </div>

                  <div className={`mt-auto px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] text-center border ${task.severity === 'critical' ? 'bg-red-50 border-red-100 text-red-600' : 'bg-slate-50 border-slate-100 text-slate-500'}`}>
         ...
         <AnimatePresence>
            {selectedTaskId && (
               <motion.div 
                  ...
                  animate={{ y: 0 }}
                  exit={{ y: 100 }}
                  className="fixed bottom-12 left-1/2 -translate-x-1/2 w-full max-w-2xl bg-slate-950 p-6 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.4)] border border-white/10 flex items-center justify-between z-50 backdrop-blur-xl"
               >
