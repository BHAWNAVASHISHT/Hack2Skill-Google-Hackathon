...
export default function AdminDashboard() {
  const { complaints, workers, logout, t } = useMockStore();
  ...
  const renderView = () => {
    switch (activeView) {
      case 'dashboard':
        return <AnalyticsView stats={stats} complaints={complaints} navigate={navigate} setActiveView={setActiveView} />;
      case 'complaints':
      case 'urgent':
      case 'scam':
  ...
  const menuItems = [
    ...
    { id: 'complaints', icon: Inbox, label: 'Complaint Box' },
    { id: 'urgent', icon: AlertCircle, label: 'Urgent', count: stats.urgent },
    { id: 'scam', icon: ShieldAlert, label: 'AI Scam Filter', count: stats.fake },
    { id: 'social', icon: Flame, label: 'Social Buster' },
    { id: 'map', icon: MapIcon, label: 'Logistics Map' },
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-6 shrink-0 z-[100] sticky top-0 text-white">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-black tracking-tighter">VERITAS</span>
        </div>
        <div className="flex items-center gap-4">
           ...
           <div className="w-10 h-10 bg-slate-800 rounded-xl border border-slate-700 flex items-center justify-center font-black text-primary shadow-sm">
             AC
           </div>
        </div>
      </header>
...
function AnalyticsView({ stats, complaints, navigate, setActiveView }: { stats: any, complaints: any[], navigate: any, setActiveView: (view: View) => void }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-1">
         <h1 className="text-3xl font-black text-slate-900 tracking-tighter">Strategic Intelligence</h1>
         <p className="text-sm font-medium text-slate-400 uppercase tracking-widest">Real-time Command Analytics</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Incident Volume', value: stats.total, sub: '+12% Trend', icon: Activity, color: 'text-primary' },
          { label: 'Security Clearance', value: '99.1%', sub: 'DigiLocker Auth', icon: Cpu, color: 'text-emerald-500' },
          { label: 'Field Capacity', value: `${Math.round((stats.activeWorkers / (stats.activeWorkers + stats.freeWorkers || 1)) * 100)}%`, sub: `${stats.activeWorkers} Units Deployed`, icon: Users, color: 'text-orange-500' },
          { label: 'Resolution TTL', value: '4.2h', sub: 'Optimal Range', icon: TrendingUp, color: 'text-indigo-500' }
        ].map((item, i) => (
          <div key={i} className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/50 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         <div className="lg:col-span-2 bg-white rounded-[3rem] p-8 border border-slate-200 shadow-xl shadow-slate-200/30 min-h-[450px] flex flex-col gap-6">
            <div className="flex items-center justify-between">
               <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Incident Frequency Map (Past 7 Days)</h3>
               <div className="flex gap-4">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase text-primary font-bold"><span className="w-2 h-2 rounded-full bg-[#f97316]"></span> Infra</div>
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase text-[#3b82f6] font-bold"><span className="w-2 h-2 rounded-full bg-[#3b82f6]"></span> Water</div>
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase text-[#10b981] font-bold"><span className="w-2 h-2 rounded-full bg-[#10b981]"></span> Power</div>
               </div>
            </div>
         ...
         <div className="bg-slate-900 rounded-[3rem] p-8 text-white shadow-2xl flex flex-col gap-8">
            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-primary">Intelligence Alerts</h3>
            <div className="flex flex-col gap-6">
               {[
                 { msg: 'Cluster detected in Sector 4: Water Failure', time: '12m ago', priority: 'high' },
                 { msg: 'System flagged redundant report in HSR', time: '45m ago', priority: 'low' },
                 { msg: 'Workforce deficit in South Zone: 2 units', time: '1h ago', priority: 'critical' }
               ].map((alert, i) => (
                 <div key={i} className="flex gap-4">
                    <div className={`w-1 h-10 rounded-full shrink-0 ${alert.priority === 'critical' ? 'bg-red-500' : 'bg-primary'}`}></div>
                    <div>
                       <p className="text-xs font-bold leading-relaxed">{alert.msg}</p>
                       <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">{alert.time}</span>
                    </div>
                 </div>
               ))}
            </div>
            <button className="mt-auto w-full py-4 bg-slate-800 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-slate-700 hover:bg-slate-700 transition-all text-slate-400">View Event Logs</button>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
         <div className="bg-white rounded-[3rem] p-8 border border-slate-200 shadow-xl shadow-slate-200/30 flex flex-col gap-6 font-sans">
            <div className="flex items-center justify-between">
               <div>
                  <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Local Needs Heatmap</h3>
         ...
         <div className="bg-white rounded-[3rem] p-8 border border-slate-200 shadow-xl shadow-slate-200/30 flex flex-col gap-6 font-sans">
            <div className="flex items-center justify-between">
               <div>
                  <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">AI Trend Insights</h3>
                  <p className="text-xl font-black text-slate-900 tracking-tighter mt-1">Problem Evolution Hub</p>
               </div>
            ...
            <div className="p-6 bg-slate-950 rounded-[2rem] text-white space-y-4 relative overflow-hidden">
               ...
               <div className="flex items-center justify-between mt-4">
                  <button className="text-[10px] font-black uppercase text-primary tracking-widest flex items-center gap-2 hover:translate-x-1 transition-transform">
                     Optimize Resource Allocation <ChevronRight className="w-3 h-3" />
                  </button>
               </div>
            </div>
...
function ComplaintsTableView({ complaints, title, navigate }: { complaints: any[], title: string, navigate: any }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        ...
        <div className="flex-1 max-w-xl relative">
           ...
           <input 
              ...
              placeholder="Scan sector records by title or track ID..." 
              value={searchQuery}
      ...
      <div className="bg-white rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden flex flex-col">
            ...
            <table className="w-full border-collapse">
               <thead>
                  <tr className="border-b border-slate-100 text-left text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                     <th className="px-8 py-6">Incident Data</th>
                     <th className="px-8 py-6">Ministry / Dept</th>
                     <th className="px-8 py-6">Tactical Location</th>
                     <th className="px-8 py-6">AI Audit</th>
               ...
               <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={6} className="py-32 text-center font-black text-slate-300 uppercase tracking-widest italic">No matching reports in sector files.</td></tr>
                  ) : (
                    filtered.map(c => (
      ...
      <div className="flex-1 bg-white rounded-[3rem] border border-slate-200 shadow-2xl relative overflow-hidden group border-2">
         ...
         <div className="absolute inset-0 bg-[#0f172a] flex items-center justify-center">
            <div className="absolute inset-0 opacity-40">
                <img 
                  ...
                  alt="High Res Urban Topography" 
                  className="w-full h-full object-cover grayscale brightness-200 contrast-125"
                />
                <div className="absolute inset-0 bg-slate-900/60" />
            </div>

            {/* Realistic Topography Layers */}
         ...
         <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            ...
            {(mapFilter === 'all' || mapFilter === 'workers') && workers.map((w, i) => (
               ...
               >
                  <div className={`w-12 h-12 ${w.status === 'assigned' ? 'bg-emerald-500 shadow-emerald-500/50' : 'bg-slate-700 shadow-slate-900/50'} rounded-[1.5rem] flex flex-col items-center justify-center text-white shadow-2xl border-2 border-white cursor-pointer transition-all hover:scale-110 group/work`}>
         ...
         <div className="absolute bottom-8 inset-x-8 pointer-events-none flex justify-center">
            <div className="bg-slate-950/90 backdrop-blur-xl p-6 rounded-[2.5rem] border border-white/20 text-white shadow-2xl pointer-events-auto flex items-center gap-10 min-w-[600px]">
               <div className="flex items-center gap-6">
                  <div className="w-12 h-12 bg-primary/20 rounded-2xl flex items-center justify-center text-primary">
                     <Activity className="w-7 h-7" />
                  </div>
                  <div className="flex flex-col">
                     <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">Delhi Telemetry</span>
                     <span className="text-sm font-black text-white uppercase tracking-[0.1em]">NCR - Tactical Corridor</span>
                  </div>
               </div>
               ...
               <div className="flex-1 flex justify-around items-center">
                  <div className="flex flex-col items-center">
                     <span className="text-[10px] font-black text-emerald-500">{workers.length} Units</span>
                     <span className="text-[8px] font-bold text-slate-500 uppercase">Synchronized</span>
                  </div>
                  <div className="flex flex-col items-center">
                     <span className="text-[10px] font-black text-orange-500">{complaints.filter(c => c.severity === 'critical').length} Active</span>
                     <span className="text-[8px] font-bold text-slate-500 uppercase">High Priority</span>
                  </div>
                  <div className="flex flex-col items-center">
                     <span className="text-[10px] font-black text-slate-300">12.5k Nodes</span>
                     <span className="text-[8px] font-bold text-slate-500 uppercase">IOT Verified</span>
                  </div>
               </div>
               ...
               <div className="flex gap-3">
                  <button 
                    ...
                    className="px-5 py-3 bg-slate-800 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-700 transition-colors border border-slate-700"
                  >
                    Audit Flow
                  </button>
                  <button 
                    ...
                    className="px-5 py-3 bg-primary rounded-2xl text-[9px] font-black uppercase tracking-widest hover:shadow-lg hover:shadow-orange-500/20 transition-all flex items-center gap-2"
                  >
        ...
        {showAudit && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-white w-full max-w-2xl rounded-[3rem] p-10 relative overflow-hidden flex flex-col gap-8 shadow-2xl"
            >
              <div className="flex justify-between items-center">
                <div className="flex flex-col gap-1">
                  <h2 className="text-3xl font-black italic tracking-tighter uppercase leading-none text-slate-900">System Audit Flow</h2>
                  <p className="text-slate-500 text-xs font-black uppercase tracking-widest tracking-[0.2em]">Verified Event Logs</p>
                </div>
                <button onClick={() => setShowAudit(false)} className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-900 transition-colors">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  { time: '14:20', event: 'Tactical Assign', det: 'Unit Delta assigned to Mission T-901', type: 'system' },
                  { time: '13:45', event: 'Alert Escalation', det: 'Critical Severity detected in Sector 4', type: 'alert' },
                  { time: '12:10', event: 'Social Buster', det: 'Accountability post triggered for Mission T-882', type: 'social' },
                  { time: '10:30', event: 'System Sync', det: 'Regional database synchronized with central hub', type: 'sync' }
                ].map((a, i) => (
                  <div key={i} className="flex gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                    <div className="text-[10px] font-black text-slate-400 font-mono mt-1">{a.time}</div>
                    <div className="flex flex-col gap-1">
                      <div className="text-xs font-black uppercase text-slate-900">{a.event}</div>
                      <div className="text-[10px] text-slate-500 font-medium">{a.det}</div>
                    </div>
                  </div>
                ))}
              </div>
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {workers.map(w => {
           ...
           return (
             <div key={w.id} className="bg-white rounded-[2.5rem] p-8 mt-4 border border-slate-200 shadow-xl shadow-slate-200/40 flex flex-col gap-8 relative overflow-hidden transition-all hover:border-primary/30">
                <div className="flex items-start justify-between">
                   <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-2xl bg-slate-950 text-white flex items-center justify-center font-black text-xl shadow-lg border border-white/10">
                        {w.name.charAt(0)}
                      </div>
                      <div>
                         <div className="font-black text-sm uppercase tracking-tight text-slate-900">{w.name}</div>
                         <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{w.department}</div>
                      </div>
                   </div>
                   <div className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${w.status === 'free' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-orange-50 border-orange-100 text-primary animate-pulse'}`}>
                      {w.status}
                   </div>
                </div>

                <div className="space-y-4">
                   <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 border-b border-slate-50 pb-2">Active Assignment</div>
                   {w.status === 'assigned' && currentTask ? (
                     <div className="flex flex-col gap-3">
                        <div className="flex items-start gap-4">
                           <div className="w-2 h-2 rounded-full bg-primary mt-1.5 shrink-0"></div>
                           <div>
                              <div className="text-xs font-black text-slate-800 uppercase tracking-tight leading-tight">{currentTask.title}</div>
                              <div className="text-[10px] text-slate-400 font-bold uppercase mt-1">Assignment: <span className="text-primary font-black">{currentTask.assignmentType || 'MANUAL'}</span></div>
                           </div>
                        </div>
                        <div className="flex items-center gap-3 ml-6 text-[10px] font-black uppercase tracking-widest text-slate-400">
                           <MapIcon className="w-3.5 h-3.5" /> {currentTask.address?.substring(0, 20)}...
                        </div>
                     </div>
                   ) : (
                     <p className="text-xs text-slate-300 font-bold italic uppercase">Awaiting tactical mission profile.</p>
                   )}
                </div>
                ...
                <div className="mt-auto pt-4 flex gap-2">
                   <button 
                      ...
                      className="flex-1 py-3 bg-slate-50 border border-slate-100 rounded-xl font-black text-[10px] text-slate-500 uppercase tracking-widest hover:bg-slate-100 transition-all"
                   >
                      Profile
                   </button>
                   <button 
                      onClick={() => navigate(`/admin/workforce/reroute/${w.id}`)}
                      className="flex-1 py-3 bg-primary/5 border border-primary/10 rounded-xl font-black text-[10px] text-primary uppercase tracking-widest hover:bg-primary/10 transition-all font-sans"
                   >
                      Re-route
                   </button>
                </div>
