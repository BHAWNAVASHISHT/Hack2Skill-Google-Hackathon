    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans pb-32">
      ...
      <main className="flex-1 p-6 flex flex-col gap-8 max-w-[600px] mx-auto w-full">
         ...
         <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-5 rounded-[2rem] border border-slate-200 shadow-xl shadow-slate-200/40">
              <div className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Queue Size</div>
              <div className="text-3xl font-black text-slate-900">{tasks.length}</div>
            </div>
            <div className={`p-5 rounded-[2rem] border shadow-xl transition-all ${tasks.length > 0 ? 'bg-orange-50 border-orange-100 shadow-orange-100' : 'bg-emerald-50 border-emerald-100 shadow-emerald-100'}`}>
              <div className={`${tasks.length > 0 ? 'text-orange-800' : 'text-emerald-800'} text-[10px] font-black uppercase tracking-widest mb-1`}>Protocol</div>
              <div className={`text-xs font-black flex items-center gap-2 uppercase ${tasks.length > 0 ? 'text-primary' : 'text-emerald-600'}`}>
                <span className={`w-2 h-2 rounded-full animate-pulse ${tasks.length > 0 ? 'bg-primary' : 'bg-emerald-500'}`}></span>
                {tasks.length > 0 ? 'Tactical Ops' : 'Standby Mode'}
              </div>
            </div>
         </div>
                ...
                {tasks.length === 0 ? (
                  <div className="bg-white py-20 rounded-[3rem] border-2 border-dashed border-slate-200 flex flex-col items-center gap-6 text-center px-10">
                      ...
                      <div className="space-y-1">
                        <h3 className="font-black text-slate-900 uppercase text-sm tracking-tight text-xl">Queue Sanitized</h3>
                      ...
                      {tasks.map(task => (
                        ...
                        >
                           {task.severity === 'critical' && (
                             <div className="absolute top-0 right-0 h-7 bg-red-600 text-[10px] font-black text-white uppercase tracking-widest px-6 flex items-center rounded-bl-3xl">Critical Priority</div>
                           )}
                           ...
                           >
                              <CheckCircle2 className="w-6 h-6" /> Confirm Neutralization
                           </button>
                        </div>
                      ))}
                  </div>
      ...
      <div className="fixed bottom-6 left-6 right-6 z-[60]">
        <div className="bg-slate-900 p-4 rounded-[2.5rem] border border-slate-700 shadow-2xl flex items-center justify-between max-w-[550px] mx-auto">
           <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-primary text-white flex items-center justify-center font-black text-lg shadow-lg shadow-orange-900/40 border border-white/20">
