    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="w-10 h-10 bg-slate-100 border-2 border-white ring-1 ring-slate-200 rounded-full flex items-center justify-center font-bold text-primary">
      ...
      <main className="max-w-6xl mx-auto w-full p-6 flex flex-col gap-10">
        ...
        <section className="mt-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            <div>
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Authenticated User Session</div>
              <h1 className="text-4xl font-black tracking-tighter text-slate-900">Welcome, {profile?.name?.split(' ')[0]}.</h1>
              <p className="text-slate-500 font-medium mt-2">Citizens can drive city improvement through active participation.</p>
            </div>
            <button 
              onClick={() => navigate('/citizen/file')}
              className="btn-primary px-8 py-4 shadow-xl shadow-orange-200 text-lg flex items-center gap-3"
            >
              <Plus className="w-6 h-6" /> File New Complaint
            </button>
          </div>
        </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Submissions', val: stats.total, icon: FileText, color: 'text-slate-400', bg: 'bg-white' },
            { label: 'In Progress', val: stats.pending, icon: Clock, color: 'text-amber-500', bg: 'bg-white' },
            { label: 'Solved', val: stats.solved, icon: CheckCircle2, color: 'text-emerald-500', bg: 'bg-white' },
            { label: 'Verified Status', val: 'Active', icon: User, color: 'text-primary', bg: 'bg-orange-50' },
          ].map((s, i) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="space-y-4">
              {citizenComplaints.length === 0 ? (
                <div className="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-slate-200">
                   ...
                   <button onClick={() => navigate('/citizen/file')} className="mt-4 text-primary font-bold hover:underline">Report an issue now</button>
                </div>
              ) : (
                citizenComplaints.map(c => (
                  ...
                  >
                     ...
                     <div className="flex items-center gap-5">
                        ...
                        <div>
                           <div className="text-lg font-bold text-slate-900 group-hover:text-primary transition-colors">{c.title}</div>
          ...
          <div className="flex flex-col gap-6">
             ...
             <div className="p-6 bg-secondary text-white rounded-2xl shadow-xl space-y-4 relative overflow-hidden">
                ...
                <div className="text-xs font-bold text-amber-400 uppercase tracking-[0.2em]">Platform Awareness</div>
             ...
             >
                 ...
                 <div className="flex flex-col gap-1">
                    ...
                    <h3 className="text-lg font-black text-white tracking-tight">Social Buster Feed</h3>
                 </div>
             ...
             <div className="p-6 bg-white border border-slate-200 rounded-2xl space-y-4 shadow-sm">
                <div className="flex items-center justify-between">
                   <h4 className="text-xs font-black uppercase tracking-widest text-slate-900">System Logs</h4>
