...
export default function FileComplaint() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="flex-1 p-6 flex flex-col gap-8 max-w-2xl mx-auto w-full mb-12">
        <div className="bg-white rounded-[2.5rem] p-6 sm:p-10 border border-slate-200 shadow-xl shadow-slate-200/50 flex flex-col gap-10">
          {/* Mode Tabs */}
          <div className="flex bg-slate-100 p-1.5 rounded-3xl">
            <button 
              type="button"
              onClick={() => setMode('ai')}
              className={`flex-1 py-3 px-4 rounded-2xl font-black text-[10px] sm:text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${mode === 'ai' ? 'bg-white text-primary shadow-lg shadow-orange-100' : 'text-slate-400 hover:text-slate-600'}`}
            >
            ...
            <button 
              ...
              className={`flex-1 py-3 px-4 rounded-2xl font-black text-[10px] sm:text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${mode === 'manual' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
            >
              {t('manualInput')}
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-10">
            ...
            <div className="flex flex-col gap-4">
               ...
               <div 
                 {...getRootProps()} 
                 className={`border-2 border-dashed rounded-[2rem] p-8 sm:p-16 text-center cursor-pointer transition-all relative overflow-hidden group ${isDragActive ? 'border-primary bg-orange-50/50' : 'border-slate-200 hover:border-primary/40 bg-slate-50/30'}`}
               >
                  <input {...getInputProps()} />
                  {preview ? (
                    <div className="relative inline-block">
                      <img src={preview} alt="Preview" className="w-48 sm:w-56 h-48 sm:h-56 object-cover rounded-3xl shadow-2xl border-4 border-white" />
                    ...
                    <div className="flex flex-col items-center gap-6">
                      <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white shadow-xl shadow-slate-200 rounded-[1.5rem] flex items-center justify-center text-primary group-hover:scale-110 transition-transform duration-500">
                         <Upload className="w-8 h-8" />
                      </div>
                      <div className="flex flex-col gap-1">
                        <p className="font-black text-slate-900 text-base sm:text-lg tracking-tight">{t('fileNew')}</p>
                        <p className="text-[10px] sm:text-xs text-slate-400 font-medium max-w-[240px] mx-auto leading-relaxed">AI will automatically decode classification and severity from your upload.</p>
                      </div>
                    </div>
                  )}
               </div>
            ...
            <div className="flex flex-col gap-4">
               ...
               <textarea 
                 ...
                 className="input-field resize-none h-40 p-6 text-sm"
               />
            </div>
            ...
            >
               ...
               >
                  <div className="flex flex-col gap-4">
                     ...
                     <h2 className="text-3xl font-black italic tracking-tighter uppercase leading-none text-slate-900">Irregularity Detected</h2>
                     <p className="text-slate-500 text-sm font-medium leading-relaxed">
                        Veritas AI has detected a potential violation in your report context.
                     </p>
                  </div>

                  <div className="p-6 bg-red-50 rounded-2xl border border-red-100 flex flex-col gap-3">
                     <div className="text-[10px] font-black uppercase tracking-widest text-red-600">AI Flag Reason:</div>
                     <p className="text-sm font-bold text-red-900">{validationResult?.reason}</p>
                     
                     <div className="mt-2 pt-4 border-t border-red-100">
                        <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Suggestion:</div>
                  ...
                  <div className="flex flex-col gap-3">
                     <button 
                        ...
                        className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-800 transition-all"
                     >
                        Refine Report
                     </button>
                     <button 
                        ...
                        className="w-full py-4 text-slate-400 font-bold uppercase tracking-widest text-[9px] hover:text-red-500 transition-all"
                     >
