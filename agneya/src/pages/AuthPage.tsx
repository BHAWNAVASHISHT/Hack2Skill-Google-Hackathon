...
export default function AuthPage() {
  ...
  const handleLogin = async () => {
    ...
    setTimeout(() => {
      login(role, role === 'admin' ? 'Admin Officer' : (role === 'worker' ? 'Field Worker' : 'Citizen Name'));
      navigate(`/${role}`);
      setLoading(false);
      setVerifying(false);
    }, 2000);
        ...
        >
          <div className="mb-12 flex justify-between items-start">
            <div>
              ...
              <p className="text-slate-400 font-medium text-sm">Select your operational clearance and language node.</p>
            </div>
            <button 
              onClick={() => navigate('/')}
